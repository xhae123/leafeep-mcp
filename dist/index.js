...
server.tool("list_exams", "내 문제 세트 목록을 조회합니다.", {}, async () => {
    const sets = await api("/api/instructor/question-sets");
    if (sets.length === 0) {
        return {
            content: [{ type: "text", text: "생성된 문제 세트가 없습니다." }],
        };
    }
    const lines = sets.map((s) => `- [${s.id}] ${s.title} (${s.questionCount}문제, ${s.createdAt.slice(0, 10)})`);
    return {
        content: [
            {
                type: "text",
...
server.tool("get_results", `시험의 전체 데이터를 조회합니다. 제출 현황, 문제별 학생 응답, 채점 결과, 정답률 통계를 모두 포함합니다.
...
server.tool("close_exam", "시험을 마감합니다 (제출 잠금). 마감 후 채점을 시작할 수 있습니다.", {
...
server.tool("list_students", `내 학생 목록을 조회합니다. 모든 시험에서 제출한 학생 이름을 중복 없이 보여줍니다.
각 학생의 응시 횟수, 채점된 시험 수, 평균 점수율(0~1)을 포함합니다.`, {}, async () => {
...
server.tool("get_student_history", `특정 학생의 전체 시험 이력을 조회합니다. 시험별로 점수, 정답/오답 수, 문제별 응답과 채점 결과를 모두 포함합니다.
